...
module.exports = class Ripple {
    ...
    async writeTransaction(data) {
        try {
            ...
            logger.info(`Maximal transaction fee set to '${currentTransConst}'.`);
            ...
            const transaction = createTransactionObject(this.source.address, this.target.address, data);
            ...
            logger.info(`Transaction '${data}' submitted.`);
            ...
            logger.error(`Transaction '${data}' submitting failure - ${err.message}`);
    ...
    async readTransaction(hash) {
        try {
            ...
            logger.info(`Ripple transaction '${hash}' acquired.`);
            ...
            logger.error(`Transaction '${hash}' read failure - ${err.message}`);
 ...
 *
 */
function createTransactionObject(sourceAddress, targetAddress, data) {
    return {
        source: {
